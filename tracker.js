        ...
        async runKeywordAudit() {
            ...
            try {
                ...
                if (!this.apiKeys.perplexity && !actualRAGContext) {
                    alertMsg += `⚠️ [경고] 실시간 웹 검색(RAG) 수집이 일시적인 프록시 차단 또는 네트워크 에러로 인해 실패했습니다. (※ 실시간 트렌드 데이터는 안정화 기간 동안 베타(Beta) 서비스로 운영하며, 데이터 수집 규모와 이용률이 낮은 01:00~06:00에는 제한적으로 서비스 됩니다.)\n\nLLM 모델이 최신 정보를 반영하지 못해 언급률이 0%로 산출되었을 수 있으니, 대시보드의 [실시간 AI 원본 답변 보기] -> [실시간 검색 컨텍스트 (RAG)] 탭에서 상세 오류 로그를 확인해 주십시오.\n\n`;
                }
